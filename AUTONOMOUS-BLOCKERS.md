...
0.0.12 HUD lifecycle hotfix, 2026-08-24: no source-repair blocker. The explicit lifecycle state machine and deterministic regression suite are implemented and green. Fresh save-backed HUD qualification is blocked because the current Windows profile contains zero exact `KBP_AUTOMATION_BASELINE` and zero exact `KBP_AUTOMATION_WORKING` fixtures; unrelated automation and personal campaign saves are protected and will not be substituted. Guarded no-save performance/catalog work, deterministic packaging, documentation, commit, and guarded branch push remain actionable. No merge, tag, install, or release publication is authorized.
