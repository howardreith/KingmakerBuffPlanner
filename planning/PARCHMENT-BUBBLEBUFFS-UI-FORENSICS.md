...
The primary controls are Search, Configured only, Show hidden, Reset, and Advanced Filters. Default ordering is alphabetical with all supported non-hidden buffs regardless of current availability. Duration/source/availability categories live in a closed drawer. The footer contains actions only; Settings owns the sole `Casting mode` control. `CASTING SOURCE` shows an automatic/caster/availability summary and keeps preference, enable/disable, spellbook identity, and cap controls under `Advanced Casting Source`. Raw primary labels and `CAP ANY` are rejected by source validation. Phase D passed source 30/30, behavior/protocol 63/63, runtime filesystem 7/7 at its checkpoint, deployment WhatIf 5/5, and package 4/4 before commit `c733b71`. The later pre-activation recovery regression raised runtime filesystem coverage to 8/8 without changing presentation or mechanics.

## Screenshot-driven refinement and final automation

The first live parchment screenshot (`ui-polish-0.0.6-instant-2`) proved that exact native frame/card sprites were unsafe when stretched far beyond their source prefab proportions. They remain inventoried, but large panels and cards now use centralized parchment fills and burgundy/gold borders; exact background, button, portrait, toggle, and font candidates remain optional references. The next screenshot removed ornamental streaking and restored button contrast.

Final versioned physical runs `ui-polish-0.0.7-animated-1` and `ui-polish-0.0.7-instant-1` each record 11 blueprint icons, zero missing-icon fallbacks, one Casting mode control, zero retired labels, the complete native-resolution summary, and five hashed views. Both pass 71/71, confirm Bless, preserve the profile assignment after reload, and restore Mods exactly. This evidence qualifies automation, not subjective visual acceptance.
