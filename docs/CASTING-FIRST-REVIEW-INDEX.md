...
| Interaction/execution | **Interaction sequence LIVE-PASSED** (run `casting-ws-author-033000`): browse without mutation, three single-target castings from capable casters, focused edit, Undo, deliberate re-edit, Save, close, production-route reopen with exact IDs/order — through canonical session commands (labeled direct session calls). Native submission remains disabled (`DisabledCastingDispatchBoundary`); physical-input acceptance of the launcher seam and native aesthetic pass (review §5) remain open. |
