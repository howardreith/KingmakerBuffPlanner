...
function Read-MethodIl([Reflection.MethodBase]$Method) {
    ...
    while ($position -lt $bytes.Length) {
        ...
        if (-not $opCodes.ContainsKey($key)) {
            throw "Unknown IL opcode 0x$($key.ToString('x4'))."
        }
...
try {
    ...
    foreach ($entry in $contracts.GetEnumerator()) {
        ...
        foreach ($methodName in @($entry.Value)) {
            ...
            [array]$methods = if ($methodName -eq '.ctor') {
                $type.GetConstructors($flags)
            } else {
                $type.GetMethods($flags) | Where-Object Name -ceq $methodName
            }
            if ($methods.Count -eq 0) {
                throw "Method not found: $($entry.Key)::$methodName"
            }
